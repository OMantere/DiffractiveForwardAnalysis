...
GammaGammaLL::analyzeMCEventContent( const edm::Event& iEvent )
{
  ...
  for ( unsigned int i = 0; i < genPartColl->size(); ++i ) {
    ...
    if ( fabs(genPart->pdgId()) == 2000013 && evt_.nGenRMuonCand < ggll::AnalysisEvent::MAX_GENSLEPTONS) {
      ...
      evt_.GenRMuonCand_e[evt_.nGenRMuonCand] = genPart->energy();
      evt_.nGenRMuonCand++;
    }
    ...
    if ( fabs(genPart->pdgId()) == 2000011 && evt_.nGenRMuonCand < ggll::AnalysisEvent::MAX_GENSLEPTONS) {
      ...
      evt_.GenREleCand_e[evt_.nGenREleCand] = genPart->energy();
      evt_.nGenREleCand++;
    }
  ...
  TLorentzVector slr1, slr2;  // Only using right handed sleptons at the moment
  TLorentzVector chi10_1, chi10_2;
  // electron+muon
  ...
  // dielectron
    ...
    slr2.SetPtEtaPhiE( evt_.GenREleCand_pt[1], evt_.GenREleCand_eta[1], evt_.GenREleCand_phi[1], evt_.GenREleCand_e[1] );
    foundGenCandPairInEvent = true;
  }
  // dimuon
    ...
    slr2.SetPtEtaPhiE( evt_.GenRMuonCand_pt[1], evt_.GenRMuonCand_eta[1], evt_.GenRMuonCand_phi[1], evt_.GenRMuonCand_e[1] );
    foundGenCandPairInEvent = true;
  }
  if ( foundGenCandPairInEvent ) {
    ...
    evt_.GenSLRPair_mass = slr_pair.M();
    evt_.GenSLRPair_pt = slr_pair.Pt();
    evt_.GenSLRPair_eta = slr_pair.Eta();
    evt_.GenSLRPair_phi = slr_pair.Phi();
    evt_.GenChi10Pair_mass = chi10_pair.M();
    evt_.GenChi10Pair_pt = chi10_pair.Pt();
    evt_.GenChi10Pair_eta = chi10_pair.Eta();
    evt_.GenChi10Pair_phi = chi10_pair.Phi();

    double dphi = fabs( l1.Phi()-l2.Phi() );
    double slr_dphi = fabs( slr1.Phi()-slr2.Phi() );
    double chi10_dphi = fabs( chi10_1.Phi()-chi10_2.Phi() );
    // dphi lies in [-pi, pi]
    while ( dphi < -M_PI ) { dphi += 2.*M_PI; }
    while ( dphi >  M_PI ) { dphi -= 2.*M_PI; }
    while ( slr_dphi < -M_PI ) { slr_dphi += 2.*M_PI; }
    while ( slr_dphi >  M_PI ) { slr_dphi -= 2.*M_PI; }
    while ( chi10_dphi < -M_PI ) { chi10_dphi += 2.*M_PI; }
    while ( chi10_dphi >  M_PI ) { chi10_dphi -= 2.*M_PI; }
    evt_.GenPair_dphi = dphi;
    evt_.GenSLRPair_dphi = slr_dphi;
    evt_.GenChi10Pair_dphi = chi10_dphi;

    evt_.GenPair_dpt = fabs( l1.Pt()-l2.Pt() );
    evt_.GenPair_3Dangle = l1.Angle( l2.Vect() )/M_PI;
    evt_.GenSLRPair_dpt = fabs( slr1.Pt()-slr2.Pt() );
    evt_.GenSLRPair_3Dangle = slr1.Angle( slr2.Vect() )/M_PI;
    evt_.GenChi10Pair_dpt = fabs( chi10_1.Pt()-chi10_2.Pt() );
    evt_.GenChi10Pair_3Dangle = chi10_1.Angle( chi10_2.Vect() )/M_PI;
  }
}
