...
vector <vector<long int>> passed_orig;
...
public:
    ...
    void run() {
        ...
        for (int i = 0; i < n_files; i++) {
            ...
            pass_frac_orig.push_back(vector<double>(0));
            for (int j = 0; j < n_vars; j++) {
                ...
                pass_frac_orig.back().push_back(((double) passed_orig[i][j] / (double) n_events[i]));
            }
...
//            VariableCut("pair_dphi", -2.5, 2.5, -3.14, 3.14, false),
            VariableCut("deltaR", 0, 2.5, 0, 5),
            VariableCut("pair_dphi", 0, 2.5, 0, 5),
...
//            VariableCut("xiP2", 0.02, 0.15, 0,1, false)
        },
        {true},
